    ...
    fun setup(view: View) {
        ...
        spinnerSmsMode.onItemSelectedListener =
            ...
            override fun onItemSelected(
                ...
                // وضع غير "off" (full/source):
                // يتطلب إحدى قناتي الاستقبال لكي يعمل فعلاً:
                ...
                if (ContextCompat.checkSelfPermission(
                    ...
                    if (NateqNotificationListener.isPermissionGranted(
                        fragment.requireContext()
                    ) &&
                        ...
                        fragment.view?.announceCompat(
                            fragment.getString(R.string.sms_reading_via_nls)
                        )
                    } else {
                        pendingSmsMode = mode
                        runCatching {
                            fragment.smsPermLauncher.launch(
                                android.Manifest.permission.RECEIVE_SMS
                            )
                        }
                    }
                ...
                // تنبيه سياسة أندرويد 17: رسائل OTP تُحجب 3 ساعات أولى بعد
                // التفعيل.
                fragment.view?.announceCompat(
                    fragment.getString(R.string.sms_otp_block_hint)
                )
                onStatusChanged()
            }
        ...
        seekSmsVolume.setOnSeekBarChangeListener(
            ...
            override fun onStopTrackingTouch(seekBar: SeekBar) {
                runCatching {
                    settings.setSmsReadingVolume(seekBar.progress / 100f)
                }
