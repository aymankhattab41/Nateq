...
internal class PunctuationStep(
    ...
    private val allEntries: List<Pair<Pattern, String>> = listOf(
        ...
        // لخطوة الأرقام لتنطق «ناقص خمسة» وليس «شرطة خمسة» — كانت الاستبدال
        // السابق يسبق NumberStep فيفسد قراءة الحساب ودرجات الحرارة. لا
        // يُعوَّض تشكيل التتابع «--» إلا بنطقٍ واحد (لدى المتبوعة رقماً
        // تبقى كما هي للمعالج العددي).
        Pattern.compile("-{1,2}(?!\\s*\\d)") to " شرطة ",
        Pattern.compile(Pattern.quote("–")) to " شرطة ",
        Pattern.compile(Pattern.quote("—")) to " شرطة "
    )
