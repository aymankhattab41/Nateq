...
class AnnouncementSpeaker(
    ...
    /** **دمجُ المقاطعِ المتعاقبةِ ذاتِ الصوتِ الواحدِ** (بندُّ إزالةِ
     *  السكتاتِ بينَ كلماتِ الجملةِ المختلطةِ الكتاباتِ): عندَ نطقِ نصٍّ
     *  مختلطِ الكتاباتِ (عربي+إنجليزي معاً مثلاً) يُقسِّمُه
     *  [LanguageSegmenter] إلى مقاطعَ صغيرةٍ (كلمةً كلمةً أحياناً إن
     *  تباينتِ السكربتاتُ) فيصلُ المحرّكُ كلَّ مقطعٍّ بنداءِ speak
     *  منفصلٍّ فيُدخِلُ محرّكُ TTS فجوةَ استحواذٍّ ظاهرةً (~ثانيةٍّ) بينَ
     *  كلمتَيِ الجملةِ الواحدةِ. الدمجُ: مقطعانِ متتاليانِ متقاربانِ في
     *  (المعدل/النبرة/المستوى) وعلى **صوتٍّ واحدٍّ** (voiceId متساوٍّ أو
     *  كلاهما فارغٌّ = المحرّكُ النظاميُّ يتولّى المزجَ الداخليَّ للكتابتينِ)
     *  يُدمَجانِ نصَّاً واحداً يُنطقُ بنداءِ speak واحدٍّ متصلٍّ — فيختفي
     *  الصمتُ بينَ كلماتِ الجملةِ وأماميَّاً (بندُّ 17). لا دمجَ بينَ
     *  صوتينِ مختلفينِ أبداً: إن خصَّ المستخدمُ صوتاً عربياًّ وآخرَ
     *  إنجليزياًّ (voiceId مختلفانِ مخصصانِ) تبقى المقاطعُ منفصلةً
     *  فيُحافَظُ على لغتِ كلِّ مقطعٍّ وصوتِه.
     *
     * يشترطُ التطابُقُ بدقَّةٍّ: voiceId متساوٍّ، وrate/pitch/volume
     *  متساويةٌّ (ضمنَ دقَّةٍّ معقولةٍّ للفاصلةِ العائمةِ بتقريبِ 0.02).
     *  ملاحظةٌّ مقصودةٌّ: لا نشترطُ تساويَ locale هنا لأنَّ النصَّ
     *  المختلطِ الكتاباتِ على الصوتِ النظاميِّ الواحدِ يستفيدُ من قدرةِ
     *  محرّكِ النظامِ الحديثِ على توزيعِ اللغاتِ داخلَ النداءِ الواحدِ
     *  تلقائياً — وهي قفزةُ الجودةِ التي يطلبُها المستخدمُ (بندُّ
     *  ب.txt 10.2/10.3: لا فجواتِ~ثانيةٍّ بينَ كلماتِ الإعلانِ). */
    private fun mergeAdjacentSameVoice(
        ...
        for (unit in units) {
            ...
            if (last != null &&
                ...
                // الجملةِ المختلطةِ مقطعاً واحداً متصلاً بلا فجوةِ~ثانيةٍّ.
