 ...
 *
 * ## متى يُستدعى؟
 * على المقطع اللاتيني المتراكم حين يبلغ كلمتين فأكثر (بند اللغة الثانية —
 * كان الحدُّ ثلاثاً فيُحرَم مقتطفُ الكلمتين من كشفٍ صحيح)؛ الكلمةُ المفردة
 * تبقى محلَّ لبسٍ فتذهب لسقوطٍ محافظ (اللغة الثانية التي يختارها المستخدم).
 */
object LatinLanguageDetector {
    ...
    /** أقل عدد كلماتٍ يُبرّر الكشف — كلمتان الآن (بند اللغة الثانية):
     *  خُفِّضت من ثلاثٍ وضاَفته بثقةٍ أعلى ورُفع [MIN_CONFIDENCE] تعويضاً،
     *  وكلمةٌ واحدة تبقى محلّ لبسٍ يسقط محافظاً على اللغة الثانية. */
    internal const val MIN_WORDS = 2
    ...
    /** حدّ أدنى لثقة الفائز حتى لا «يربح» لغةٌ بلا أثرٍ يذكر — رُفع من 1.00
     *  إلى 1.10 (بند اللغة الثانية) تعويضاً عن خفض [MIN_WORDS] إلى كلمتين:
     *  مقتطفُ الكلمتين بمفردةٍ وظيفيةٍ واحدة (0.6) لا يُحسم، وبمفردتين
     *  وظيفيتين أو حرفِ تشكيلٍ قاطع يُحسم. */
    private const val MIN_CONFIDENCE = 1.10f
    ...
    private fun score(
        ...
        // التغطية مُطبَّعة على حجم ملمح اللغة نفسه (بند اللغة الثانية): كانت
        // تقسم على عدد ثنائيات النص فقط فتربح الملامحُ الأطولُ أساساً بسبب
        // كتلتها الأكبر لا انسجامها — القسمة الآن على (عدد الثنائيات × كتلة
        // الملمح) تعيدها نسبةً 0..1 عادلةً بين اللغات متساويةِ الطول.
        val coverage = if (bigrams.isEmpty()) {
            0.0
        } else {
            val profileMass = profile.bigrams.values.sum().coerceAtLeast(1)
            mass / (bigrams.size * profileMass)
        }
