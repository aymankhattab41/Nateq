...
/**
 * يغطّي استخراج PCM من ملفات WAV في [SystemVoiceProvider] عبر
 * Reflection (الطريقة والنتيجة خاصّان). التركيز على الملفات التالفة التي
 * كانت توقع المؤشر في حلقة لا نهائية عند قراءة حجم خانة سالب (البت 31
...
class SystemVoiceProviderWavExtractTest {
    ...
    private fun extractPcm(file: File): Any {
        ...
        val method = SystemVoiceProvider::class.java.getDeclaredMethod(
            "extractPcm", File::class.java
        )
        method.isAccessible = true
        return method.invoke(provider, file)
            ?: error("extractPcm returned null")
    }
    ...
    private fun intField(result: Any, name: String): Int =
        (fieldOf(result, name) as? Int) ?: error("missing Int field $name")

    private fun byteField(result: Any, name: String): ByteArray {
        val value = fieldOf(result, name) as? ByteArray
        return value ?: error("missing byte[] field $name")
    }
    ...
    private fun riffWith(chunks: ByteArray): ByteArray =
        "RIFF".toByteArray(Charsets.US_ASCII) +
            leInt(4 + chunks.size) +
            "WAVE".toByteArray(Charsets.US_ASCII) + chunks
    ...
    private fun validWav(): ByteArray {
        ...
        return riffWith(
            chunk("fmt ", fmt) + chunk("data", byteArrayOf(1, 2, 3, 4))
        )
    }
    ...
    /**
     * خانة volume بحجم 0xFFFFFFF8 (−8) كانت تبقّي المؤشر على نفس
     * الموضع إلى الأبد.
     */
    @Test
    ...
    /**
     * 0x80000000 (البت 31 مضبوطاً) كان يقفز بالمؤشر إلى موضع
     * سالب ويُرمى على الأرجح.
     */
    @Test
    ...
    /**
     * 0xFFFFFFFF (−1) — مع بُنى تالفة كان يتراكم نزولاً ويعيد
     * تكرار مواضع سابقة.
     */
    @Test
