...
1. حدّث `changelog_text` (عربي + إنجليزي) ببنود آخر 3 التزامات (انظر أدناه).
...
- **المحتوى محصور في آخر 3 التزامات فقط**: `changelog_text` في الملفين يعرض
  حصراً بنود المستجدات لتغييرات آخر 3 التزامات فقط (الجديدة أولاً بترتيب
  عكسي، ولا يُضاف سجل تاريخي تراكمي ولا بنود أقدم) — يُستخرج من `git log`
  ويُستبدل بالكامل عند كل دفع، كل بند منه يمثّل أحدث إنجاز.
- **سبب الحصر إلزامي تقنياً**: أي سلسلة موارد واحدة تتجاوز **32767 بايت
  UTF-8** يقلّصها aapt2 إلى النص الحرفي `STRING_TOO_LARGE` (عُثر عليها في
  بند المستجدات وفُحصت العتبة تجريبياً: 32000 بايت تنجح و33000 تنهار).
  الحصر بآخر 3 التزامات يبقي السلسلة صغيرة دائماً فيتوقف الانهيار نهائياً.
  رفع رقم الإصدار يبقى حصراً عبر `scripts/release.ps1`.
- **ملاحظات Release على GitHub تأتي من نفس المصدر**: `scripts/release.ps1`
  ...
  لاحظ أن البنود في الصدارة تُلتقط (أحدث 5 بنود) لهذه الملاحظات، لذا تبقى
  أحدث 3 التزامات دوماً في مقدمة السلسلة.
...
5. مستجدات — تحديث `changelog_text` في الملفين (واجب قبل أي دفع، حصرٌ بآخر 3 التزامات).
