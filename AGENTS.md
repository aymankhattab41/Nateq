...
### الكود الميت يُحذف بلا استئذان (سجّلها المدير)
**أي كود ميت/لا يُستخدم في الكود الرئيسي: دوال أو خصائص أو معاملات توقيعات أو
فروع `when`/تعليقات توصف ما لم يعد موجوداً — يحذفه الوكيل فور اكتشافه دون الرجوع
إلى المدير** على أن يُذكر الحذف في تقرير النتائج، بشرط ألا يضر بالسلوك:
- لا يُحذف إلا ما تحقّق الوكيل بلا استخدام له في الـ main **وفي الاختبارات** (طرف
  استخدامً أي أثر يمتنع عن الحذف أو يُرقِّع العلامة المستخدمة بدلاً منه).
- التعليقات القديمة التي تصف خياراً/سلوكاً أُزيل («تلقائي», «بدون محرك», أخطاء
  صياغة نصية) تُصحَّح أو تُحذف مع تنفيذ التغيير الذي أزاح موضوعها — لا تبقى
  وصيةً لماضي الكود.
- الموارد (`strings.xml`) بلا أي إشارة في الكود تُحذف من الملفين عربياً وإنجليزياً.
- البناء والتشغيل (اختبار الوحدة على الأقل) يبقى أخضر بعد كل حذف.
